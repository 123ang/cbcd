...
| Exit accessibility | Preference for routes with better access to reachable exits. | Path-level normalized Manhattan distance to nearest exit, reported separately for explanation. | `exit_access_score`; not added per step to avoid overlapping with A* heuristic. |
